...
/*   Updated: 2022/04/29 00:53:32 by seujeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	set_env_error(char *new_argv, t_data *data)
{
	if (ft_strchr(new_argv, '=') == NULL)
	{
		print_execute_err_3("env", new_argv, MSG_FILE_NOT_FOUND_ERR, data);
		g_exit_status = 127;
		return (1);
	}
	if (new_argv && new_argv[0] == '=')
	{
		print_execute_err_3("env", "setenv =", MSG_INVALID_ARG_ERR, data);
		g_exit_status = 1;
		return (1);
	}
	return (0);
...
int	pt_env(char **envp, char *prefix, char **new_argv, t_data *data)
{
	...
	while (new_argv[++i])
	{
		if (set_env_error(new_argv[i], data))
			return (0);
	}
	i = -1;
	while (envp[++i])
